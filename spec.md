...
Cursor encodes `(createdAt, id)` of the last item. Query:
```sql
SELECT * FROM posts
...
function getOrCreateNickname(postId, userId):
    ...
    // Step 1: Try to insert a new nickname
    ...
    // Step 2: If insert did nothing, user already has a nickname
    if result.rowsAffected == 0:
        return SELECT nickname FROM anonymous_nicknames
               WHERE post_id = :postId AND user_id = :userId

    // Step 3: Insert succeeded — return the new nickname
    return nickname

    // Step 4: If insert fails on UNIQUE(post_id, nickname) constraint
    // (another user got the same random nickname for this post)
    // → catch the constraint violation, retry with a new nickname
    // → max 3 retries, then fail with 409 Conflict
```
...
| 8 | **Nickname pool exhaustion** | After 3 failed retries due to nickname collision, fall back to `"{Adjective} {SeaCreature} {randomNumber}"` (e.g., "Brave Whale 42"). Guarantees uniqueness beyond the 2,500 base combinations. |
